...
+ (GDataXMLNode*) getNode: (GDataXMLNode*) element withName: (NSString*) name {
	for(GDataXMLNode* child in [element children]) {
		if([child respondsToSelector:@selector(name)] &&  [[child name] hasSuffix:name]) {
			return (GDataXMLNode*)child;
		}
	}
	for(GDataXMLNode* child in [element children]) {
		GDataXMLNode* el = [Soap getNode: (GDataXMLElement*)child withName: name];
		if(el != nil) { return el; }
	}
	return nil;
}
...
+ (NSArray*) findNodes: (GDataXMLNode*) node xpath: (NSString*) xpath {
	NSError* error;
	return [node nodesForXPath: xpath error: &error];
}

// Finds a single node with the given XPath query.
+ (GDataXMLNode*) findNode: (GDataXMLNode*) node xpath: (NSString*) xpath {
	NSArray* a = [Soap findNodes: node xpath: xpath];
	if(a != nil && [a count] > 0) {
		return (GDataXMLNode*)[a objectAtIndex:0];
	}
	return nil;
}

// Deserialize an object as a generic object
+ (id) deserialize: (GDataXMLNode*) element{
...
+(id)deserializeAsDictionary:(GDataXMLNode*)element {
...
+ (id) deserialize: (GDataXMLNode*) element forObject: (NSObject*) object {
	NSError* error;
	NSObject* value;
...
+ (NSString*) getNodeValue: (GDataXMLNode*) node withName: (NSString*) name {

	// Set up the variables
	if(node == nil || name == nil) { return nil; }
	GDataXMLNode* child = nil;
	
	// If it's an attribute get it
	if([node isKindOfClass: [GDataXMLElement class]])
	{
		child = [(GDataXMLElement*)node attributeForName: name];
		if(child != nil) {
...
+(id)objectFromNode:(GDataXMLNode*)node {
	NSMutableArray* children = [NSMutableArray arrayWithArray:[node children]];
	if([node isKindOfClass:[GDataXMLElement class]]) {
		[children addObjectsFromArray:[(GDataXMLElement*)node attributes]];
	}
	if([children count] > 0) {
