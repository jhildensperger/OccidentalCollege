...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection {
	...
	GDataXMLDocument* doc = [[GDataXMLDocument alloc] initWithData: self.receivedData options: 0 error: &error];
	if(doc == nil) {
		[self handleError:error];
		return;
	}
	...
	if([fault hasFault]) {
		...
		GDataXMLNode* element = [[Soap getNode: [doc rootElement] withName: @"Body"] childAtIndex:0];
