...
{
    _place = place;
    _coordinate = CLLocationCoordinate2DMake(place.lat, place.lon);
    return self;
}
...
{
    if(!locationView) {
        locationView = (ExampleLocationView*)[aMapView dequeueReusableAnnotationViewWithIdentifier:@"ExampleLocationView"];
        if(!locationView)
